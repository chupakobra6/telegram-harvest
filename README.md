...
Команда пишет plain UTF-8 transcript в `--output`, а в stdout — ASR contract v2 с `profile_id`, `validation_status`, backend descriptor, diagnostics и timings. `transcribe-file --check` проверяет текущие runtime/model paths без обработки медиа и возвращает `validation_status: runtime-ready`. Профили разделены явно: обычный вход — `short-message-v1`, доверенная короткая запись — `trusted-speech-v1`, OBS long-form — `trusted-long-form-v2`. OBS Interview Pipeline использует только этот публичный контракт, поэтому Whisper profile, VAD, Metal/decode validation и post-filter обновляются только здесь, в `internal/transcribe`.

Для короткой доверенной записи `--assume-speech` пропускает только whole-file Silero gate. Для длинной OBS-записи `--trusted-long-form` bounded Silero-окнами находит первую и последнюю речь, сохраняет секундный lead-in и одним запросом включает нативный timestamped long-form decode Whisper. Внутренний декодер сам выбирает границы окон, переносит подтверждённый контекст и продвигается по timestamp-токенам; внешних 120-секундных чанков и текстовой склейки нет. Harvest проверяет duration, монотонность timestamps и достижение последней найденной речи с двухсекундным допуском; только после этого возвращается `validation_status: coverage-validated`. Это гарантия структурной целостности и покрытия хвоста, а не оценка WER/CER. Модель `large-v3-turbo-q5_0`, Metal, русский decode profile и terminal post-filter остаются теми же и по-прежнему принадлежат этому пакету. Обычные Telegram-команды эти флаги не используют и сохраняют whole-file gate с `no_timestamps=true`.
