...
- Инструмент передаёт агентам выбранные учебные чаты, личный дневной контекст и полные приватные архивы зарегистрированных аккаунтов.
...
- Профили `main` и `study` используют соответственно `TG_HARVEST_DAILY_*` и `TG_HARVEST_STUDY_*`. Зарегистрированные аккаунты заимствуют только API ID/hash выбранного источника, а не его пользовательскую сессию или номер. Не добавляй алиасы переменных.
